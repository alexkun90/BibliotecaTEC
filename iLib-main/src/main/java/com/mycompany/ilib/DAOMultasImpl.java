...
public class DAOMultasImpl extends Database implements DAOMultas{
    ...
    public void eliminar(int multaid) throws Exception {
        try {
            this.Conectar();
            PreparedStatement st = this.conexion.prepareStatement("DELETE FROM books WHERE id_multa = ?");
            st.setInt(1, multaid);
            st.executeUpdate();
        } catch (Exception e) {
            throw e;
    ...
    public List<Multas> listar() throws Exception {
        ...
        try {
            this.Conectar();
            PreparedStatement st = this.conexion.prepareStatement("SELECT * FROM multas_table ORDER BY id DESC");

            lista = new ArrayList<>();
            ResultSet rs = st.executeQuery();
            while (rs.next()) {
                Multas multa = new Multas();
                multa.setId_multa(rs.getInt("id"));
                multa.setId_users(rs.getInt("user_id"));
    ...
    public Multas getMultaById(int multaId) throws Exception {
        ...
        try {
            this.Conectar();
            PreparedStatement st = this.conexion.prepareStatement("SELECT * FROM multas WHERE id = ? FETCH FIRST 1 ROWS ONLY");
            st.setInt(1, multaId);
