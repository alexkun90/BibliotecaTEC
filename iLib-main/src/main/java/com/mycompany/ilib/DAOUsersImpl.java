...
public class DAOUsersImpl extends Database implements DAOUsers {
    ...
    public List<Users> listar(String name) throws Exception {
        ...
        try {
            this.Conectar();
            String query = name.isEmpty() ? "SELECT * FROM users" : "SELECT * FROM users WHERE name LIKE ?";
    ...
    public Users getUserById(int userId) throws Exception {
        ...
        try {
            this.Conectar();
            PreparedStatement st = this.conexion.prepareStatement("SELECT * FROM users WHERE id = ? FETCH FIRST 1 ROWS ONLY");
            st.setInt(1, userId);
