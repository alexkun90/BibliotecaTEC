...
public class DAOLendingsImpl extends Database implements DAOLendings {
    ...
    public void registrar(Lendings lending) throws Exception {
        try {
            this.Conectar();
            PreparedStatement st = this.conexion.prepareStatement("INSERT INTO lendings_table(user_id, book_id, date_out) VALUES(?,?,?)");
    ...
    public void modificar(Lendings lending) throws Exception {
        try {
            this.Conectar();
            PreparedStatement st = this.conexion.prepareStatement("UPDATE lendings_table SET user_id = ?, book_id = ?, date_out = ?, date_return = ? WHERE id = ?");
            st.setInt(1, lending.getUser_id());
            st.setInt(2, lending.getBook_id());
            st.setString(3, lending.getDate_out());
    ...
    public Lendings getLending(Users user, Books book) throws Exception {
        ...
        try {
            this.Conectar();
            PreparedStatement st = this.conexion.prepareStatement("SELECT * FROM lendings_table WHERE user_id = ? AND book_id = ? AND date_return IS NULL ORDER BY id DESC FETCH FIRST 1 ROWS ONLY");
    ...
    public List<Lendings> listar() throws Exception {
        ...
        try {
            this.Conectar();
            PreparedStatement st = this.conexion.prepareStatement("SELECT * FROM lendings_table ORDER BY id DESC");
